...
export class ActivityService {
  ...
  async update(userId: number, id: number, input: Partial<ActivityInput>) {
    ...
    const nextDate = input.recordDate ? dayjs(input.recordDate).format('YYYY-MM-DD') : activity.recordDate;
    const user = await this.userService.findById(userId);
    const factor = await this.factorService.findEffectiveAt(nextCategory, nextSubType, user.region, nextDate);
